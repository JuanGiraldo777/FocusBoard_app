...
const pomodoroSessionService = {
    ...
    try {
      ...
      if (data.roomId) {
        ...
        if (memberCheck.rows.length === 0) {
          throw createAppError("User is not a member of this room", 403);
        }
      }

      await client.query(
        `INSERT INTO pomodoro_sessions 
         (user_id, room_id, task_label, duration_seconds, status, started_at, ended_at)
  ...
  async getTodaySessions(userId: number) {
    const result = await db.query(
      `SELECT id, task_label, duration_seconds AS duration, status, started_at, ended_at
       FROM pomodoro_sessions 
       WHERE user_id = $1 
